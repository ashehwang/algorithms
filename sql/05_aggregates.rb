...
def africa_gdp
  ...
  execute(<<-SQL)
  select SUM(gdp) as total_gdp from countries group by continent having continent = 'Africa'
  SQL
end
...
def group_population
  ...
  execute(<<-SQL)
  select sum(population) as total_population from countries where name in ('France', 'Germany', 'Spain')
  SQL
end
